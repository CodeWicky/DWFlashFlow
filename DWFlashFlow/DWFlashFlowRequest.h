...
///   3.预处理回调发生在请求发送之前。
///   4.预处理回调中可以对请求做最后处理，可以修改除预处理回调和拦截器回调之外的所有属性（事实上我并不建议你在预处理中修改除了请求参数外的其他属性）
...
@property (nonatomic ,copy) DWFlashFlowDestinationCallback destination;
...
//Actual request preprocessor
///实际请求的预处理回调
@property (nonatomic ,copy) DWFlashFlowProcessorBlock actualPreprocessor;
...
@property (nonatomic ,strong) DWFlashFlowAbstractRequest * request;
